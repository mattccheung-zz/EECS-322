...
namespace LA {
    ...
    struct typeInst : seq<type, sSeps, var> {};
    struct assignInst : seq<var, wSeps, opAssign, wSeps, s> {};
    struct assignOpInst : seq<var, wSeps, opAssign, wSeps, t, wSeps, op, wSeps, t> {};
    struct labelInst : label {};
    struct brLabelInst : seq<kwBr, sSeps, label> {};
    struct brVarLabelInst : seq<kwBr, sSeps, t, sSeps, label, sSeps, label> {};
    struct returnInst : kwReturn {};
    struct returnVarInst : seq<kwReturn, sSeps, t> {};
    ...
    struct fBody : seq<one<'{'>, plus<seq<wSeps, inst>>, wSeps, one<'}'>> {};
    struct f : seq<fHead, wSeps, fBody> {};
    struct p : seq<plus<seq<wSeps, f>>, wSeps> {};
    ...
    struct action<brVarLabelInst> {
        static void apply(const input &in, Program &p) {
            ...
            p.functions.back()->instructions.push_back(new BranchInst(operands[0], operands[1], operands[2]));
            operands.clear();
        }
    };
    ...
    struct action<returnVarInst> {
        static void apply(const input &in, Program &p) {
            assert(operands.size() == 1);
            p.functions.back()->instructions.push_back(new ReturnInst(operands[0]));
            operands.clear();
        }
    };
