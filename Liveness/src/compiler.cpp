...
bool eval_inst(string &operand1, string &operand2, L2::Operator_Type &op) {
    int64_t a = stoll(operand1), b = stoll(operand2);
    return op == L2::Operator_Type::LQ ? a < b : (op == L2::Operator_Type::LEQ ? a <= b : a == b);
}
            ...
            switch (inst->operators.front()) {
                case L2::Operator_Type::MOVQ:
                    ...
                    if (inst->operators.size() == 1) {
                        ...
                        if (operand2[0] != 'r' && operand3[0] != 'r') {
                            ...
                            if (inst->operators[1] == L2::Operator_Type::LQ) {
                                output << "\tsetge ";
                            } else if (inst->operators[1] == L2::Operator_Type::LEQ) {
                                output << "\tsetg ";
                            } else {
                                output << "\tsete ";
                            }
                ...
                case L2::Operator_Type::CJUMP:
                    ...
                    if (operand2[0] != 'r' && operand3[0] != 'r') {
                        ...
                        output << "\tcmpq " << get_opd(operand2) << ", " << get_opd(operand3) << endl;
                        if (inst->operators[1] == L2::Operator_Type::LEQ) {
                            output << "\tjg ";
                        } else if (inst->operators[1] == L2::Operator_Type::LQ) {
                            output << "\tjge ";
                        } else {
                            output << "\tje ";
                        }
                    ...
                    break;
                case L2::Operator_Type::LABEL:
                    output << get_label(inst->operands[0]);
                    break;
                case L2::Operator_Type::GOTO:
                    output << "\tjmp " << get_call_opd(inst->operands[0]);
                    break;
                case L2::Operator_Type::RETURN:
                ...
                case L2::Operator_Type::MEM:
                    ...
                    break;
                case L2::Operator_Type::INC:
                    output << "\tinc " << get_opd(inst->operands[0]);
                    break;
                case L2::Operator_Type::DEC:
